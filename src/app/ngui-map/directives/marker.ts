...
export class Marker extends BaseMapDirective implements OnInit {
  ...
  setPosition(): void {
    if (!this['position']) {
      ...
      console.log('setting marker position from lat long: ', this['position']);
      const latlong = this['position'].split(', ');
      const latitude = parseFloat(latlong[0]);
      const longitude = parseFloat(latlong[1]);
      this.mapObject.setPosition(new google.maps.LatLng(latitude, longitude));

      // TODO: Commented by Anirudh because I don't want to get the address. I just want to mark the lat long points.
